...
from XLNET_BILSTM_CRF import modeling, xlnet, model_utils
import sentencepiece as spm
from XLNET_BILSTM_CRF.prepro_utils import preprocess_text, encode_ids
from tensorflow.contrib import rnn, crf
from tensorflow.contrib.crf import crf_log_likelihood
from tensorflow.contrib.layers.python.layers import initializers
import matplotlib.pyplot as plt
import time
from XLNET_BILSTM_CRF.utils import f1_score, format_result, get_tags, format_tags, new_f1_score
...
# Optimization
flags.DEFINE_float("learning_rate", default=3e-8, help="initial learning rate")  # 3e-5
...
flags.DEFINE_float("lr_layer_decay_rate", default=0.00005,                     # 0.75
...
flags.DEFINE_string("xlnet_config", default="../XLNET_BILSTM_CRF/chinese_xlnet_base_L-12_H-768_A-12/xlnet_config.json", help="xlnet config file dir")
flags.DEFINE_string("init_checkpoint", default="../XLNET_BILSTM_CRF/chinese_xlnet_base_L-12_H-768_A-12/xlnet_model.ckpt", help="xlnet model init checkpoint")
flags.DEFINE_string("spm", default="../XLNET_BILSTM_CRF/chinese_xlnet_base_L-12_H-768_A-12/spiece.model", help="spiece model file")
...
class Model():
    ...
    def train(self):
        from XLNET_BILSTM_CRF.xlnet_data_utils import XLNetDataUtils
        ...
        f = open("../XLNET_BILSTM_CRF/model/data_map.pkl", "wb")
        cPickle.dump(data, f)
        f.close()
        ...
        print("names of tags:\t", self.train_data.tag_map.keys())
            ...
            with tf.device("/gpu:0"):
                ...
                for i in range(self.max_epoch):
                    ...
                    for batch in self.train_data.get_batch():
                        ...
                        if steps % 100 == 0:
                            self.xlnet_evaluate(sess, "W")
                            self.xlnet_evaluate(sess, "R")
                            self.xlnet_evaluate(sess, "S")
                            self.xlnet_evaluate(sess, "M")
                        if steps % 1000 == 0:
    ...
    def predict(self):
        ...
        with tf.Session() as sess:
            ...
            while True:
                ...
                org = get_tags(paths[0], "W", self.tag_map)
                org_entity = format_result(org, text, "W")
                per = get_tags(paths[0], "R", self.tag_map)
                per_entity = format_result(per, text, "R")
                loc = get_tags(paths[0], "S", self.tag_map)
                loc_entity = format_result(loc, text, "S")
                loc = get_tags(paths[0], "M", self.tag_map)
                loc_entity = format_result(loc, text, "M")
